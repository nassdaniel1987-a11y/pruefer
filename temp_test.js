...
const calcScore = (nameA, nameB) => {
  ...
  // Penalty logik
  const unmatched = Math.abs(tA.length-matches.length)+avail.length;
  const penalty = unmatched * 25;
  ...
  return { score, reason, avg: Math.round(avg), penalty, unmatched, matches_arr: matches, tA, tB, avail };
};

console.log("TEST 1: Carlos David Valentin Mögel vs Valentin Carlos");
console.log(calcScore("Carlos David Valentin Mögel", "Valentin Carlos"));

console.log("\nTEST 2: Carmen Perez Ferandes vs Fernandes Carmen Perez");
console.log(calcScore("Carmen Perez Ferandes", "Fernandes Carmen Perez"));
