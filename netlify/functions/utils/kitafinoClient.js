// Client für das kitafino-Einrichtungsportal.
//
// Herausgelöst aus kitafino.js, damit der HTTP-Endpunkt und der tägliche
// Automatiklauf dieselbe Quelle nutzen. Hier steckt das gesamte Wissen über
// die Eigenheiten des Portals — siehe CLAUDE.md, Abschnitt "kitafino integration".
...
// ─── Buchungen für einen Zeitraum holen ────────────────────
/**
 * Meldet sich an, lädt die Stammliste und holt die Buchungen Tag für Tag.
 *
 * @param {{von:string, bis:string, projektId:string, spur?:string[]}} opts
 * @returns {Promise<{eintraege:Array, von:string, bis:string, tageAbgefragt:number,
 *                    tageOhneEssen:number, kinderGesamt:number, eintraegeMitId:number}>}
 */
const holeBuchungen = async ({ von, bis, projektId, spur = null }) => {
  if (von > bis) {
    const err = new Error('Startdatum liegt nach dem Enddatum');
    err.code = 'BAD_RANGE';
    throw err;
  }
  ...
  if (tage.length > MAX_DAYS) {
    const err = new Error(`Zeitraum zu groß (${tage.length} Tage, max. ${MAX_DAYS})`);
    err.code = 'BAD_RANGE';
    throw err;
  }
    ...
    for (const r of results) {
      ...
      for (const b of r.bestellungen) {
        ...
        eintraege.push({
          ...
          // Klasse kommt bewusst nicht von kitafino: die Spalte "Gruppe/Klasse"
          // enthält dort für alle Nutzer den Einrichtungsnamen. Liste A ist die Quelle.
          klasse: '',
          datum: r.datum,
          menu: b.menu,
  ...
  if (tageOhneTabelle === tage.length) {
    const err = new Error('Keine Tagesansicht erhalten — Portal nicht erreichbar oder Sitzung ungültig');
    err.code = 'NO_DATA';
    throw err;
  }
  ...
  return {
    ...
    tageOhneEssen: tageOhneEssen.length,
    kinderGesamt: kinder.size,
    eintraegeMitId: eintraege.filter(e => e.kitafino_id).length,
  };
};

module.exports = { login, fetchRoster, holeBuchungen, MAX_DAYS };
